...
dataset_dir= "/media/sdd/gaoxingyu/Project1" # 数据集根目录
...
class create_dataset_train:
    def __init__(self, dataset_dir):  
        ...
        dataset_dir_to = os.path.join(dataset_dir, 'ADNI1_MRI_PET_ADNC_255')
    ...
    def __getitem__(self, index):
        ...
        out = img['data']
        data = np.array(out[1]).astype(np.float32) 
...
class create_dataset_test:
    def __init__(self, dataset_dir):  
        ...
        dataset_dir_to = os.path.join(dataset_dir, 'ADNI2_MRI_PET_ADNC_255')
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(dataset_train):
        ...
        if i%5 == 0 or i == step_size_train -1:
