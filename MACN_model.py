...
__all__ = ["DenseNet21plusTransformer", ]
...
def conv5x5x5(in_channels, out_channels, stride=1, padding=1, has_bias=False):
...
class Attention(nn.Cell):
    def __init__(self, features):
        ...
        self.conv1 = conv1x1x1(features, features)
        self.conv3 = conv3x3x3(features, features)
...
class _DenseLayer(nn.Cell):
    ...
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.relu1 = nn.ReLU()
        self.conv1_3 = conv3x3x3(num_input_features, bn_size*growth_rate)
        self.conv1_5 = conv5x5x5(num_input_features, bn_size*growth_rate)


        self.norm2 = nn.BatchNorm3d(bn_size*growth_rate)
        self.relu2 = nn.ReLU()
        self.conv2_3 = conv3x3x3(bn_size*growth_rate, growth_rate)
        self.conv2_5 = conv5x5x5(bn_size*growth_rate, growth_rate)

        # nn.Dropout in MindSpore use keep_prob, diff from Pytorch
        self.p = 1.0 - drop_rate
        self.dropout = nn.Dropout(self.p)

        self.ATT1 = Attention(bn_size*growth_rate)

        self.ATT2 = Attention(growth_rate)

    def construct(self, features):
        bottleneck1_3 = self.conv1_3(self.relu1(self.norm1(features)))
        bottleneck1_5 = self.conv1_5(self.relu1(self.norm1(features)))

        #Attention
        bottleneck1_3 = self.ATT1(bottleneck1_3)*bottleneck1_3
        bottleneck1_5 = self.ATT1(bottleneck1_5)*bottleneck1_5

        bottleneck1_c = bottleneck1_3 + bottleneck1_5

        new_features2_3 = self.conv2_3(self.relu2(self.norm2(bottleneck1_c)))
        new_features2_5 = self.conv2_5(self.relu2(self.norm2(bottleneck1_c)))

        new_features2_3 = self.ATT2(new_features2_3)*new_features2_3
        new_features2_5 = self.ATT2(new_features2_5)*new_features2_5

        new_features = new_features2_3 + new_features2_5



        # bottleneck = self.conv1_3(self.relu1(self.norm1(features)))
        # new_features = self.conv2_3(self.relu2(self.norm2(bottleneck)))
...
class _Transition(nn.Cell):
    ...
    """
    def __init__(self, num_input_features, num_output_features, Maxpool=False):
...
class Densenet(nn.Cell):
    ...
    def __init__(self, growth_rate, block_config, num_init_features=None, bn_size=4, drop_rate=0):
        ...
        self.maxpool = P.MaxPool3D(kernel_size=3, strides=2,)
        self.maxpool_end = P.MaxPool3D(kernel_size=(3, 4, 3), strides=2,)
        ...
        # Transformer
        self.mapping = nn.SequentialCell(MHSA(num_features, 3, 4, 3),nn.BatchNorm3d(num_features), nn.ReLU())
...
class macn(nn.Cell):
    ...
    def __init__(self, num_classes, include_top=False):
        ...
        self.backbone = MSAplusTRANSFORMER()
